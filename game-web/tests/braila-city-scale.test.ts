...
describe('Brăila canonical playable city scale', () => {
  it('expands gameplay distance to the owner 10x baseline without falsifying retained geographic bounds', () => {
    expect(BRAILA_PLAYABLE_SCALE_VERSION).toBe(3)
    expect(BRAILA_PLAYABLE_DISTANCE_SCALE).toBe(10)
    expect(BRAILA_PLAYABLE_DISTANCE_SCALE).toBeGreaterThanOrEqual(10)
  ...
  it('restores a true City semantic overview on compact Android landscape at 10x scale', () => {
  ...
  it('turns the old sub-minute straight crossing into a ten-minute-class city crossing at walking speed', () => {
    const walkingSpeed = 150
    expect(rawLayout.width / walkingSpeed).toBeLessThan(75)
    expect(WORLD_WIDTH / walkingSpeed).toBeGreaterThan(600)
  })
})
