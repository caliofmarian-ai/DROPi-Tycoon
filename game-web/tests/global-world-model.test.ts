...
describe('global world model', () => {
  ...
  it('allows fewer than nine nodes instead of requiring fabricated settlements', () => {
    const example = country()
    expect(validateCountryNodePolicy(example).valid).toBe(true)
    expect(example.representativeLocalities.length).toBeLessThan(MAX_REPRESENTATIVE_LOCALITIES_PER_COUNTRY)
  })
  ...
  it('permits independent external industrial and agricultural nodes', () => {
  ...
  it('maps world-clock hours to day-part presentation state', () => {
