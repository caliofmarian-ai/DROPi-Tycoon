...
export const buildCustomerReviewsLayout = (
  ...
  const panelTop = clamp(Math.round(viewport.height * 0.17), 104, 150)
  ...
  const rowsPerPage = viewport.height >= 760 ? 5 : 4
  const rowRects = buildRows(panel, rowsPerPage, 68, 7)
  ...
  return {
    ...
    summary: {
      x: centerX,
      y: clamp(Math.round(viewport.height * 0.115), 72, 102),
      fontSize: clamp(Math.round(viewport.width * 0.043), 15, 19),
    },
    panel,
    rowRects,
    rowFontSize: clamp(Math.round(viewport.width * 0.038), 13, 17),
    rowTextWrapWidth: Math.max(210, panel.width - 44),
    emptyText: {
      x: centerX,
