...
export type HireEmployeeResult =
  ...
  | {
      hired: false
      company: CompanyState
      candidate: EmployeeCandidate | null
      reason: 'unknown-candidate' | 'already-hired' | 'not-enough-money' | 'invalid-state'
      message: string
    }
...
export type OnboardingResult =
  ...
  | {
      ...
      reason: 'unknown-employee' | 'already-active' | 'invalid-status'
      message: string
    }
...
const isValidMoney = (value: number): boolean =>
  Number.isSafeInteger(value) && value >= 0

const isValidSalary = (value: number): boolean =>
  Number.isSafeInteger(value) && value >= 0
...
export const hireEmployee = (
  ...
  if (!candidate) {
    return {
      ...
      message: 'Employee candidate is not available.',
    }
  }
  ...
  if (!isValidMoney(company.money) || !isValidMoney(candidate.hireCost) || !isValidSalary(candidate.salaryPerCycle)) {
    return {
      ...
      message: 'Company or employee cost data is invalid.',
    }
  }

  if (findEmployee(company, employeeId)) {
    return {
      hired: false,
      company,
      candidate,
      reason: 'already-hired',
      message: `${candidate.name} is already part of the company.`,
    }
  }

  if (!canAfford(company.money, candidate.hireCost)) {
    return {
      hired: false,
      company,
      candidate,
      reason: 'not-enough-money',
      message: `Not enough money. Hiring ${candidate.name} costs ${candidate.hireCost}.`,
    }
  }
  ...
  if (!isValidMoney(money)) {
    return {
      ...
      candidate,
      reason: 'invalid-state',
      message: 'Hiring result is invalid.',
    }
  }
...
export const completeEmployeeOnboarding = (
  company: CompanyState,
  employeeId: string,
): OnboardingResult => {
  ...
  if (!employee) {
    return {
      ...
      message: 'Employee is not part of the company.',
    }
  }
  ...
  if (employee.status === 'Active') {
    return {
      ...
      message: `${employee.name} is already active.`,
    }
  }
  ...
  if (employee.status !== 'Onboarding') {
    return {
      ...
      message: 'Employee onboarding state is invalid.',
    }
  }

  const activatedEmployee: EmployeeState = {
    ...employee,
    status: 'Active',
  }
  ...
  return {
    ...
    company: {
      ...
      employees: company.employees.map((item) =>
        item.employeeId === employeeId ? activatedEmployee : item,
      ),
      payroll: { ...company.payroll },
    },
...
export const processSalaryCycle = (
  company: CompanyState,
  cycleId: number,
): SalaryCycleResult => {
  ...
  if (!Number.isSafeInteger(cycleId) || cycleId <= 0) {
    return {
      ...
      message: 'Salary cycle identifier must be a positive integer.',
    }
  }

  if (
    !isValidMoney(company.money) ||
    !Number.isSafeInteger(company.payroll.lastProcessedCycle) ||
    company.payroll.lastProcessedCycle < 0 ||
    eligibleEmployees.some((employee) => !isValidSalary(employee.salaryPerCycle)) ||
    !Number.isSafeInteger(totalSalary) ||
    totalSalary < 0
  ) {
    return {
      processed: false,
      ...
      chargedEmployeeIds,
      reason: 'invalid-state',
      message: 'Payroll state is invalid.',
    }
  }

  if (cycleId <= company.payroll.lastProcessedCycle) {
    return {
      processed: false,
      company,
      cycleId,
      ...
      message: `Salary cycle ${cycleId} was already processed.`,
    }
  }

  if (cycleId !== company.payroll.lastProcessedCycle + 1) {
    return {
      processed: false,
      company,
      cycleId,
      ...
      message: `Salary cycle ${company.payroll.lastProcessedCycle + 1} must be processed next.`,
    }
  }

  if (!canAfford(company.money, totalSalary)) {
    return {
      processed: false,
      company,
      cycleId,
      ...
      chargedEmployeeIds,
      reason: 'not-enough-money',
      message: `Company needs ${totalSalary} to process salary cycle ${cycleId}.`,
    }
  }
