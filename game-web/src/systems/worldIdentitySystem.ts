...
/**
 * Deterministic non-cryptographic fingerprint for local/offline identifiers only.
 * It is not authentication material and must never be treated as proof of account ownership.
 */
...
export const sanitizeWorldIdentityState = (value: unknown): WorldIdentitySanitizeResult => {
  ...
  const repaired = suppliedHeroActorId !== expectedHeroActorId
  return {
    state: {
      worldInstanceId,
      accountId,
