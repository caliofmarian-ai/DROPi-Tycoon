#!/usr/bin/env python3
"""Generate a deterministic detailed building + preview for DT-19 CI evidence."""
...
def create_material(name: str, rgba: list[float], roughness: float = 0.72) -> bpy.types.Material:
...
def add_front_window(
    ...
    add_box(f"{prefix}_glass", (x, y, z), (width, depth * 0.35, height), glass, asset_objects)
    ...
    add_box(f"{prefix}_sill", (x, y - depth * 0.28, z - outer_h / 2 - 0.05), (outer_w + 0.22, depth * 1.25, 0.10), trim, asset_objects)
...
def add_side_window(
    ...
    add_box(f"{prefix}_glass", (x, y, z), (depth * 0.35, width, height), glass, asset_objects)
    outer_w = width + 2 * frame
    outer_h = height + 2 * frame
    ...
    add_box(f"{prefix}_sill", (x + depth * 0.28, y, z - outer_h / 2 - 0.05), (depth * 1.25, outer_w + 0.22, 0.10), trim, asset_objects)
...
def validate_request_parameters(request: dict) -> dict:
    ...
    for key in ["floors", "frontBays", "sideBays"]:
        value = int(params[key])
        if value <= 0 or value > 12:
            raise RuntimeError(f"parameters.{key} must be between 1 and 12")
...
def main() -> int:
    ...
    body_mat = create_material("MAT_body", list(params["bodyColor"]), 0.78)
    trim_mat = create_material("MAT_trim", list(params["trimColor"]), 0.68)
    glass_mat = create_material("MAT_glass", list(params["glassColor"]), 0.24)
    roof_mat = create_material("MAT_roof", list(params["roofColor"]), 0.76)
    door_mat = create_material("MAT_door", list(params["doorColor"]), 0.62)

    add_box("VISUAL_body", (0.0, 0.0, body_height / 2.0), (width, depth, body_height), body_mat, asset_objects)
    ...
    for floor in range(floors):
        ...
        for bay in range(front_bays):
            ...
            add_front_window(front_index, x, front_y, z, window_width, window_height, frame, trim_depth, glass_mat, trim_mat, asset_objects)
            front_index += 1

        for bay in range(side_bays):
            y = -depth / 2.0 + (bay + 0.5) * (depth / side_bays)
            add_side_window(side_index, side_x, y, z, window_width, window_height, frame, trim_depth, glass_mat, trim_mat, asset_objects)
            side_index += 1
    ...
    add_box("VISUAL_entrance_door", (door_x, front_y - trim_depth * 0.05, door_z), (door_width, trim_depth * 0.55, door_height), door_mat, asset_objects)
    ...
    add_box("VISUAL_entrance_awning", (door_x, front_y - 0.42, door_height + 0.35), (door_outer_w + 0.45, 0.80, 0.14), trim_mat, asset_objects)

    add_box("VISUAL_front_plinth", (0.0, front_y, 0.28), (width + 0.08, trim_depth, 0.56), trim_mat, asset_objects)
    add_box("VISUAL_side_plinth", (side_x, 0.0, 0.28), (trim_depth, depth + 0.08, 0.56), trim_mat, asset_objects)
    add_box("VISUAL_front_cornice", (0.0, front_y - trim_depth * 0.12, body_height - 0.16), (width + 0.28, trim_depth * 1.25, 0.30), trim_mat, asset_objects)
    add_box("VISUAL_side_cornice", (side_x + trim_depth * 0.12, 0.0, body_height - 0.16), (trim_depth * 1.25, depth + 0.28, 0.30), trim_mat, asset_objects)
    ...
    add_gable_roof(width, depth, body_height, roof_height, overhang, roof_mat, asset_objects)
    ...
    bpy.ops.object.select_all(action="DESELECT")
    ground_mat = create_material("MAT_preview_ground", [0.16, 0.17, 0.18, 1.0], 0.92)
    bpy.ops.mesh.primitive_plane_add(size=max(width, depth) * 4.0, location=(0.0, 0.0, 0.0))
    ground = bpy.context.active_object
    ground.name = "PREVIEW_ground"
    ground.data.materials.append(ground_mat)

    bpy.ops.object.camera_add(location=(18.0, -21.0, body_height + 5.5))
    camera = bpy.context.active_object
    camera.name = "PREVIEW_camera"
    camera.data.lens = 52
    look_at(camera, (0.0, 0.0, body_height * 0.48))
    bpy.context.scene.camera = camera

    bpy.ops.object.light_add(type="AREA", location=(-5.0, -10.0, body_height + 8.0))
    ...
    bpy.ops.object.light_add(type="AREA", location=(10.0, -4.0, body_height + 3.0))
