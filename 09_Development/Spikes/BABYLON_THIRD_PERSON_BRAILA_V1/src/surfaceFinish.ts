      ...
      const observer = scene.onBeforeRenderObservable.add(() => {
        ...
        if (shadows instanceof ShadowGenerator) {
          ...
          if (hero && shadowMap) shadowMap.renderList = scene.meshes.filter(mesh => {
            ...
            return (mesh.name.startsWith('p3-car-') || mesh.name.startsWith('p3-shell-') || ['dropi-hq', 'maras-market', 'recovery-neutral-commercial-block', 'recovery-maria-shop-shell', 'customer-block', 'hero-parcel'].includes(mesh.name)) && Vector3.DistanceSquared(mesh.getBoundingInfo().boundingBox.centerWorld, hero.position) < 70 * 70
          })
        }
        publish()
      })
